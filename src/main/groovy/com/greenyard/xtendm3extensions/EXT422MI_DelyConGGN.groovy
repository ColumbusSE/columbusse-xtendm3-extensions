 ...
 * EXT422MI.DelyConGGN
 *
 * XtendM3 api connected to MWS422 - package created
 * Date	    Changed By  Description
 * 20220630	FRAZAH62    creation
 *
 * Transfer the GGN information for a M3 package, selected via an APIFlow from the GYGI DB,
 * into a text, connected to the package record MFTRNS via CRS980MI and store the TXID in MFTRNS
 */
...
public class DelyConGGN extends ExtendM3Transaction {
	...
	*/
	...
	boolean delTxtBlocklins (long txidLong) {
		...
		def callback = { Map <String, String> out ->
			if (out.error != null) {
				//mi.error(out.errorMessage);
				logger.debug("XtendM3Debug_delTxtBlocklins()- not ok - txid ${txid} kept");
				return false;
			}
			logger.debug("XtendM3Debug_delTxtBlocklins()- ok - txid set to 0l");
			return true;
		}
	 ...
	 *   KV10 - camu
	 * output
	 *   OK/NOK - result will not be used
	 */
	boolean setTextId (String txid, String cono, String whlo, String dlix, String panr, String rorc, String ridn, String ridl, String ridx, String bano, String camu) {
		logger.debug("XtendM3Debug_setTextId() Start");
		def callback = { Map <String, String> out ->
			if (out.error != null) {
				logger.debug("XtendM3Debug_setTextId- not ok");
				//mi.error(out.errorMessage);
				//return false;
	 ...
	 *   TFIL - constant value MSYTXH
	 * Output
	 *   OK/NOK - Error message on NOT ok
	 */
	boolean addTxtBlockHead (String txid) {
		logger.debug("XtendM3Debug_addTxtBlockHead() Start");
		def callback = { Map <String, String> out ->
			if (out.error != null) {
				logger.error("XtendM3Debug_addTxtBlockHead- not ok - " + out.errorMessage);
				mi.error(out.errorMessage);
				return false;
			}
			//mi.outData.put("CONO", out.get("CONO").trim());
			logger.debug("XtendM3Debug_addTxtBlockHead- ok");
		...
		//.error("${params}");
		miCaller.call("CRS980MI", "AddTxtBlockHead", params, callback);
		return true;
	}
	...
	void workOnCsvData(final String csvDat, final Map<String, String> input) {
		...
		if (arrGGN [0] == null || arrGGN [0].trim() == "") {
			...
			logger.error("XtendM3_workOncsvData no GGN code - " + errorMessage);
			return;
		}
