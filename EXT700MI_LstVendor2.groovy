...
public class LstVendor2 extends ExtendM3Transaction {
  ...
  //******************************************************************** 
  private Optional<DBContainer> findbankCIDADR(Integer CONO, String SUNO){  
    DBAction query = database.table("CIDADR").index("00").selection("SAADR1", "SATOWN", "SAPONO").build()
    def CIDADR2 = query.getContainer()
    CIDADR2.set("SACONO", CONO)
    CIDADR2.set("SASUNO", SUNO)
    CIDADR2.set("SAADTE", 10)
    CIDADR2.set("SAADID", "")
    if(query.readAll(CIDADR2, 4, addressProcessor2))  { 
  ...
  Closure<?> releasedLineProcessor = { DBContainer line ->   
    ...
    Optional<DBContainer> CIDADR2 = findbankCIDADR(company, supplier)
